...
- Updated `hex-renderer-py` to fix an edge case where `/patterns` commands could sometimes produce overlapping patterns.
...
- Fixed an internal error in `/changelog`.
...
- Internal: Set up Prometheus metrics.
